...
make_wrapper__(Goal, _) :-
    logtraceln('XXX: START make_wrapper__ ~Q.', Goal),
    forall(call(Goal),
           (logtraceln('XXX: Queueing ~Q.', Goal),
...
concludeConstructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonConstructor(Method))),
...
concludeNOTConstructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonNOTConstructor(Method))),
           not(factNOTConstructor(Method)),
           not(factConstructor(Method)),
...
concludeRealDestructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonRealDestructor(Method))),
...
concludeNOTRealDestructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonNOTRealDestructor(Method))),
           not(factRealDestructor(Method)),
           not(factNOTRealDestructor(Method)),
...
concludeDeletingDestructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonDeletingDestructor(Method))),
...
concludeNOTDeletingDestructor(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonNOTDeletingDestructor(Method))),
           not(factDeletingDestructor(Method)),
           not(factNOTDeletingDestructor(Method)),
...
concludeObjectInObject(Out) :-
    ...
    setof((OuterClass, InnerClass, Offset),
          (retract(forward_pending(user:reasonObjectInObject(OuterClass, InnerClass, Offset))),
...
concludeDerivedClass(Out) :-
    ...
    setof((DerivedClass, BaseClass, ObjectOffset),
          (retract(forward_pending(user:reasonDerivedClass(DerivedClass, BaseClass, ObjectOffset))),
...
concludeNOTDerivedClass(Out) :-
    ...
    setof((DerivedClass, BaseClass, ObjectOffset),
          (retract(forward_pending(user:reasonNOTDerivedClass(DerivedClass, BaseClass, ObjectOffset))),
           is_current(DerivedClass),
           is_current(BaseClass),
           iso_dif(DerivedClass, BaseClass),
           not(factDerivedClass(DerivedClass, BaseClass, ObjectOffset)),
...
concludeEmbeddedObject(Out) :-
    ...
    setof((OuterClass, InnerClass, ObjectOffset),
          (retract(forward_pending(user:reasonEmbeddedObject(OuterClass, InnerClass, ObjectOffset))),
           is_current(OuterClass),
           is_current(InnerClass),
           iso_dif(OuterClass, InnerClass),
...
concludeNOTEmbeddedObject(Out) :-
    ...
    setof((OuterClass, InnerClass, ObjectOffset),
          (retract(forward_pending(user:reasonNOTEmbeddedObject(OuterClass, InnerClass, ObjectOffset))),
           is_current(OuterClass),
           is_current(InnerClass),
           iso_dif(OuterClass, InnerClass),
           not(factEmbeddedObject(OuterClass, InnerClass, ObjectOffset)),
...
concludeVFTable(Out) :-
    ...
    setof(VFTable,
          (retract(forward_pending(user:reasonVFTable(VFTable))),
...
concludeVFTableWrite(Out) :-
    ...
    setof((Insn, Method, Offset, VFTable),
          (retract(forward_pending(user:reasonVFTableWrite(Insn, Method, Offset, VFTable))),
...
concludeVFTableOverwrite(Out) :-
    ...
    setof((Method, VFTable1, VFTable2, Offset),
          (retract(forward_pending(user:reasonVFTableOverwrite(Method, VFTable1, VFTable2, Offset))),
...
concludeVFTableEntry(Out) :-
    ...
    setof((VFTable, VFTableOffset, Address),
          (retract(forward_pending(user:reasonVFTableEntry(VFTable, VFTableOffset, Address))),
...
concludeNOTVFTableEntry(Out) :-
    ...
    setof((VFTable, VFTableOffset, Address),
          (retract(forward_pending(user:reasonNOTVFTableEntry(VFTable, VFTableOffset, Address))),
           not(factVFTableEntry(VFTable, VFTableOffset, Address)),
           not(factNOTVFTableEntry(VFTable, VFTableOffset, Address)),
           loginfoln('Concluding ~Q.',
...
concludeVFTableSizeGTE(Out) :-
    ...
    setof((VFTable, Size),
          (retract(forward_pending(user:reasonVFTableSizeGTE(VFTable, Size))),
...
concludeVFTableSizeLTE(Out) :-
    ...
    setof((VFTable, Size),
          (retract(forward_pending(user:reasonVFTableSizeLTE(VFTable, Size))),
...
concludeVirtualFunctionCall(Out) :-
    ...
    setof((Insn, Method, OOffset, VFTable, TOffset),
          (retract(forward_pending(user:reasonVirtualFunctionCall(Insn, Method, OOffset, VFTable, TOffset))),
...
concludeVBTable(Out) :-
    ...
    setof(VBTable,
          (retract(forward_pending(user:reasonVBTable(VBTable))),
...
concludeVBTableWrite(Out) :-
    ...
    setof((Insn, Method, Offset, VBTable),
          (retract(forward_pending(user:reasonVBTableWrite(Insn, Method, Offset, VBTable))),
...
concludeVBTableEntry(Out) :-
    ...
    setof((VBTable, VBTableOffset, Value),
          (retract(forward_pending(user:reasonVBTableEntry(VBTable, VBTableOffset, Value))),
...
concludeClassSizeGTE(Out) :-
    ...
    setof((Class, Size),
          (retract(forward_pending(user:reasonClassSizeGTE(Class, Size))),
           is_current(Class),
...
concludeClassSizeLTE(Out) :-
    ...
    setof((Class, Size),
          (retract(forward_pending(user:reasonClassSizeLTE(Class, Size))),
           is_current(Class),
...
concludeClassHasNoBase(Out) :-
    ...
    setof(Class,
          (retract(forward_pending(user:reasonClassHasNoBase(Class))),
           is_current(Class),
...
concludeClassHasUnknownBase(Out) :-
    ...
    setof(Class,
          (retract(forward_pending(user:reasonClassHasUnknownBase(Class))),
           is_current(Class),
           not(factClassHasUnknownBase(Class)),
           not(factClassHasNoBase(Class)),
...
concludeReusedImplementation(Out) :-
    ...
    setof(Method,
          (retract(forward_pending(user:reasonReusedImplementation(Method))),
...
concludeClassCallsMethod(Out) :-
    ...
    setof((Class, Method),
          ExistingClass^(retract(forward_pending(user:reasonClassCallsMethod(Class, Method))),
...
concludeNOTMergeClasses(Out) :-
    ...
    setof((Class1, Class2),
          (retract(forward_pending(user:reasonNOTMergeClasses_new(Class1, Class2))),
