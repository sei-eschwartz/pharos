...
insanityContradictoryNOTConstructor(Out) :-
    logwarnln('insanity thingy being recomputed'),
    reasonNOTConstructor(M),
    factConstructor(M),
    logwarnln('insanity ~Q', M),
...
sanityChecks(Out) :-
    logwarnln('sanityChecks being recomputed...'),
    insanityEmbeddedAndNot(Out);
    insanityConstructorAndNotConstructor(Out);
    insanityConstructorAndRealDestructor(Out);
    insanityConstructorInVFTable(Out);
...
weird :-
    (reasonNOTConstructor(0x61ed60) -> logwarnln('A ITS TRUE'); logwarnln('A ITS NOT TRUE')),

    ((reasonNOTConstructor_H(X), X=0x61ed60) -> logwarnln('C ITS TRUE'); logwarnln('C ITS NOT TRUE')),
    ((reasonNOTConstructor(X), X=0x61ed60) -> logwarnln('B ITS TRUE'); logwarnln('B ITS NOT TRUE')).


sanityChecks :-
    sanityChecks(Out)
    ->
        logwarnln('We failed! ... ~Q', Out),
        call(Out),
        weird,
        fail
    ;
