...
guessVirtualFunctionCall(Out) :-
    ...
    minof((Insn, Constructor, OOffset, VFTable, VOffset),
          (likelyVirtualFunctionCall(Insn, Constructor, OOffset, VFTable, VOffset),
           delay(not(factNOTConstructor(Constructor))),
           doNotGuessHelper(
...
guessVFTable(Out) :-
    ...
    osetof(VFTable,
           (possibleVFTable(VFTable),
            delay(not(factNOTVFTable(VFTable))),
...
tryDerivedClass(DerivedClass, BaseClass, Offset) :-
    countGuess,
    loginfoln('Guessing ~Q.', factDerivedClass(DerivedClass, BaseClass, Offset, unknown)),
...
likelyDeletingDestructor(DeletingDestructor, RealDestructor) :-
    ...
    delay(not(factNOTDeletingDestructor(DeletingDestructor))),
    ...
    delay(not(factNOTRealDestructor(RealDestructor))),
    true.
